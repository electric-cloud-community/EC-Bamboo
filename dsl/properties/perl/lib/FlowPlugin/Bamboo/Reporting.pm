...
sub buildDataset {
    ...
    for my $row ( reverse @$records ) {
        my %payload = (
            ...
            timestamp           => $bamboo->datetimeToUTC($row->{buildStartedTime}),
            endTime             => $bamboo->datetimeToUTC($row->{buildCompletedTime}),
            startTime           => $bamboo->datetimeToUTC($row->{buildStartedTime}),
