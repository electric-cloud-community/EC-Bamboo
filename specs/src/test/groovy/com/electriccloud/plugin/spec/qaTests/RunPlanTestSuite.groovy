...
class RunPlanTestSuite extends PluginTestHelper{
    ...
    static def TC = [
            ...
            C388141: [ids: 'C388141', description: ''],
            C388142: [ids: 'C388142', description: ''],
            C388143: [ids: 'C388143', description: ''],
            C388144: [ids: 'C388144', description: ''],
    ...
    static def expectedLogs = [
            default:     ["Request URI: http://bamboo-server:8085/rest/api/latest/result/PLANRUNKEY?expand=results.result.artifacts%2Cresults.result.labels",
                            "http://bamboo-server:8085/rest/api/latest/queue/PROJECTKEY-PLANKEY?executeAllStages=true"],
...
//            assertRecursively(planRunInfo, jobProperties[propertyName] )
        ...
        where:
        caseId     | configName   | projectKey     | planKey      | additionalBuildVariables | waitForBuild | waitTimeout | customRevision | resultFormat    | resultPropertySheet  | expectedSummary             | expectedLog
        TC.C388135 | CONFIG_NAME  | 'PROJECT'      | 'QARUNPLAN'  | ''                       | '1'          | '300'       | ''             | 'json'          | '/myJob/runResult'   | expectedSummaries.default   | expectedLogs.default
        TC.C388136 | CONFIG_NAME  | 'PROJECT'      | 'QARUNPLAN'  | ''                       | '1'          | '300'       | ''             | 'propertySheet' | '/myJob/runResult'   | expectedSummaries.default   | expectedLogs.default
        TC.C388137 | CONFIG_NAME  | 'PROJECT'      | 'QARUNPLAN'  | ''                       | '1'          | '300'       | 'master'       | 'json'          | '/myJob/runResult'   | expectedSummaries.default   | expectedLogs.customRevision
        TC.C388138 | CONFIG_NAME  | 'PROJECT'      | 'QARUNPLAN'  | ''                       | '1'          | '300'       | commitHash     | 'propertySheet' | '/myJob/runResult'   | expectedSummaries.default   | expectedLogs.customRevision
        TC.C388139 | CONFIG_NAME  | 'PROJECT'      | 'QARUNPLAN'  | 'TEST_MESSAGE=QAtestMessage' | '1'          | '300'       | ''             | 'propertySheet' | '/myJob/runResult'   | expectedSummaries.default   | expectedLogs.default
        TC.C388140 | CONFIG_NAME  | 'PROJECT'      | 'QARUNPLAN'  | ''                       | '1'          | '300'       | ''             | 'json'          | '/myJob/runResult'   | expectedSummaries.default   | expectedLogs.default
    }
